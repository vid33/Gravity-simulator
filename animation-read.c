...
void writeDetails()
{
  ...
  cout << currentLine << "\n";
  vidfile.seekg(0);
  vidfile.seekg(currentLine*(sizeof(double)) );
  vidfile.read(xyzBuffer, (object_no*3*sizeof(double)) );
  ...
  for (int i = 0; i < object_no; i++){
    ...
    for (int blah = 0; blah < 3; blah++) {
      sprintf(localValue, "%.6g", xyzBuffer[(i*3)+blah]);
      cout << xyzBuffer[i*3+blah] << "\t" << localValue << "\n";
      strcat(planetInformation, localValue);
      strcat(planetInformation, "\n");
    }
...
void animate()
{
 ...
 /*The file where all the position information is*/
  ifstream centralfile("/tmp/.Sizif");
  ...
  centralfile.seekg(currentLine*(sizeof(double)));
   
  /*get the data for the object*/
  centralfile.read(buffer, (3*object_no*sizeof(double)));
